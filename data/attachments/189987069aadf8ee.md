# Instructions

- Following Playwright test failed.
- Explain why, be concise, respect Playwright best practices.
- Provide a snippet of code with the fix, if possible.

# Test info

- Name: authentication/login.spec.js >> Authentication - Login Tests >> TC01 Verify Login with Valid Credentials
- Location: tests/authentication/login.spec.js:16:5

# Error details

```
Test timeout of 30000ms exceeded while running "beforeEach" hook.
```

```
Error: page.goto: Test timeout of 30000ms exceeded.
Call log:
  - navigating to "https://demowebshop.tricentis.com/", waiting until "load"

```

# Page snapshot

```yaml
- generic [ref=e2]:
  - generic [ref=e3]:
    - generic [ref=e4]:
      - link "Tricentis Demo Web Shop" [ref=e6]:
        - /url: /
        - img "Tricentis Demo Web Shop" [ref=e7]
      - list [ref=e10]:
        - listitem [ref=e11]:
          - link "Register" [ref=e12]:
            - /url: /register
        - listitem [ref=e13]:
          - link "Log in" [ref=e14]:
            - /url: /login
        - listitem [ref=e15]:
          - link "Shopping cart (0)" [ref=e16]:
            - /url: /cart
            - generic [ref=e17]: Shopping cart
            - generic [ref=e18]: (0)
        - listitem [ref=e19]:
          - link "Wishlist (0)" [ref=e20]:
            - /url: /wishlist
            - generic [ref=e21]: Wishlist
            - generic [ref=e22]: (0)
      - generic [ref=e24]:
        - status [ref=e25]
        - textbox [ref=e26]: Search store
        - button "Search" [ref=e27] [cursor=pointer]
    - list [ref=e29]:
      - listitem [ref=e30]:
        - link "Books" [ref=e31]:
          - /url: /books
      - listitem [ref=e32]:
        - link "Computers" [ref=e33]:
          - /url: /computers
      - listitem [ref=e34]:
        - link "Electronics" [ref=e35]:
          - /url: /electronics
      - listitem [ref=e36]:
        - link "Apparel & Shoes" [ref=e37]:
          - /url: /apparel-shoes
      - listitem [ref=e38]:
        - link "Digital downloads" [ref=e39]:
          - /url: /digital-downloads
      - listitem [ref=e40]:
        - link "Jewelry" [ref=e41]:
          - /url: /jewelry
      - listitem [ref=e42]:
        - link "Gift Cards" [ref=e43]:
          - /url: /gift-cards
    - generic:
      - generic [ref=e44]:
        - generic [ref=e45]:
          - strong [ref=e47]: Categories
          - list [ref=e49]:
            - listitem [ref=e50]:
              - link "Books" [ref=e51]:
                - /url: /books
            - listitem [ref=e52]:
              - link "Computers" [ref=e53]:
                - /url: /computers
            - listitem [ref=e54]:
              - link "Electronics" [ref=e55]:
                - /url: /electronics
            - listitem [ref=e56]:
              - link "Apparel & Shoes" [ref=e57]:
                - /url: /apparel-shoes
            - listitem [ref=e58]:
              - link "Digital downloads" [ref=e59]:
                - /url: /digital-downloads
            - listitem [ref=e60]:
              - link "Jewelry" [ref=e61]:
                - /url: /jewelry
            - listitem [ref=e62]:
              - link "Gift Cards" [ref=e63]:
                - /url: /gift-cards
        - generic [ref=e64]:
          - strong [ref=e66]: Manufacturers
          - list [ref=e68]:
            - listitem [ref=e69]:
              - link "Tricentis" [ref=e70]:
                - /url: /tricentis
        - generic [ref=e71]:
          - strong [ref=e73]: Popular tags
          - generic [ref=e74]:
            - list [ref=e76]:
              - listitem [ref=e77]:
                - link "apparel" [ref=e78]:
                  - /url: /producttag/4/apparel
              - listitem [ref=e79]:
                - link "awesome" [ref=e80]:
                  - /url: /producttag/8/awesome
              - listitem [ref=e81]:
                - link "book" [ref=e82]:
                  - /url: /producttag/10/book
              - listitem [ref=e83]:
                - link "camera" [ref=e84]:
                  - /url: /producttag/13/camera
              - listitem [ref=e85]:
                - link "cell" [ref=e86]:
                  - /url: /producttag/12/cell
              - listitem [ref=e87]:
                - link "compact" [ref=e88]:
                  - /url: /producttag/9/compact
              - listitem [ref=e89]:
                - link "computer" [ref=e90]:
                  - /url: /producttag/6/computer
              - listitem [ref=e91]:
                - link "cool" [ref=e92]:
                  - /url: /producttag/3/cool
              - listitem [ref=e93]:
                - link "digital" [ref=e94]:
                  - /url: /producttag/16/digital
              - listitem [ref=e95]:
                - link "jeans" [ref=e96]:
                  - /url: /producttag/14/jeans
              - listitem [ref=e97]:
                - link "jewelry" [ref=e98]:
                  - /url: /producttag/11/jewelry
              - listitem [ref=e99]:
                - link "nice" [ref=e100]:
                  - /url: /producttag/1/nice
              - listitem [ref=e101]:
                - link "shirt" [ref=e102]:
                  - /url: /producttag/5/shirt
              - listitem [ref=e103]:
                - link "shoes" [ref=e104]:
                  - /url: /producttag/7/shoes
              - listitem [ref=e105]:
                - link "TCP" [ref=e106]:
                  - /url: /producttag/19/tcp
            - link "View all" [ref=e108]:
              - /url: /producttag/all
      - generic [ref=e109]:
        - generic [ref=e110]:
          - strong [ref=e112]: Newsletter
          - generic [ref=e114]:
            - text: "Sign up for our newsletter:"
            - textbox [ref=e116]
            - button "Subscribe" [ref=e118] [cursor=pointer]
        - generic [ref=e119]:
          - strong [ref=e121]: Community poll
          - generic [ref=e123]:
            - strong [ref=e124]: Do you like nopCommerce?
            - list [ref=e125]:
              - listitem [ref=e126]:
                - radio "Excellent" [ref=e127]
                - text: Excellent
              - listitem [ref=e128]:
                - radio "Good" [ref=e129]
                - text: Good
              - listitem [ref=e130]:
                - radio "Poor" [ref=e131]
                - text: Poor
              - listitem [ref=e132]:
                - radio "Very bad" [ref=e133]
                - text: Very bad
            - button "Vote" [ref=e135] [cursor=pointer]
      - generic [ref=e138]:
        - generic:
          - generic:
            - link:
              - /url: https://www.tricentis.com/speed/
            - link:
              - /url: https://academy.tricentis.com
        - generic [ref=e139]:
          - heading "Welcome to our store" [level=2] [ref=e141]
          - generic [ref=e142]:
            - paragraph [ref=e143]: Welcome to the new Tricentis store!
            - paragraph [ref=e144]: Feel free to shop around and explore everything.
        - generic [ref=e145]:
          - strong [ref=e147]: Featured products
          - generic [ref=e149]:
            - link "Picture of $25 Virtual Gift Card" [ref=e151]:
              - /url: /25-virtual-gift-card
              - img "Picture of $25 Virtual Gift Card" [ref=e152]
            - generic [ref=e153]:
              - heading "$25 Virtual Gift Card" [level=2] [ref=e154]:
                - link "$25 Virtual Gift Card" [ref=e155]:
                  - /url: /25-virtual-gift-card
              - generic "824 review(s)" [ref=e156]
              - generic [ref=e159]:
                - generic [ref=e161]: "25.00"
                - button "Add to cart" [ref=e163] [cursor=pointer]
          - generic [ref=e165]:
            - link "Picture of 14.1-inch Laptop" [ref=e167]:
              - /url: /141-inch-laptop
              - img "Picture of 14.1-inch Laptop" [ref=e168]
            - generic [ref=e169]:
              - heading "14.1-inch Laptop" [level=2] [ref=e170]:
                - link "14.1-inch Laptop" [ref=e171]:
                  - /url: /141-inch-laptop
              - generic "1645 review(s)" [ref=e172]
              - generic [ref=e175]:
                - generic [ref=e177]: "1590.00"
                - button "Add to cart" [ref=e179] [cursor=pointer]
          - generic [ref=e181]:
            - link "Picture of Build your own cheap computer" [ref=e183]:
              - /url: /build-your-cheap-own-computer
              - img "Picture of Build your own cheap computer" [ref=e184]
            - generic [ref=e185]:
              - heading "Build your own cheap computer" [level=2] [ref=e186]:
                - link "Build your own cheap computer" [ref=e187]:
                  - /url: /build-your-cheap-own-computer
              - generic "841 review(s)" [ref=e188]
              - generic [ref=e191]:
                - generic [ref=e193]: "800.00"
                - button "Add to cart" [ref=e195] [cursor=pointer]
          - generic [ref=e197]:
            - link "Picture of Build your own computer" [ref=e199]:
              - /url: /build-your-own-computer
              - img "Picture of Build your own computer" [ref=e200]
            - generic [ref=e201]:
              - heading "Build your own computer" [level=2] [ref=e202]:
                - link "Build your own computer" [ref=e203]:
                  - /url: /build-your-own-computer
              - generic "378 review(s)" [ref=e204]
              - generic [ref=e207]:
                - generic [ref=e209]: "1200.00"
                - button "Add to cart" [ref=e211] [cursor=pointer]
          - generic [ref=e213]:
            - link "Picture of Build your own expensive computer" [ref=e215]:
              - /url: /build-your-own-expensive-computer-2
              - img "Picture of Build your own expensive computer" [ref=e216]
            - generic [ref=e217]:
              - heading "Build your own expensive computer" [level=2] [ref=e218]:
                - link "Build your own expensive computer" [ref=e219]:
                  - /url: /build-your-own-expensive-computer-2
              - generic "428 review(s)" [ref=e220]
              - generic [ref=e223]:
                - generic [ref=e225]: "1800.00"
                - button "Add to cart" [ref=e227] [cursor=pointer]
          - generic [ref=e229]:
            - link "Picture of Simple Computer" [ref=e231]:
              - /url: /simple-computer
              - img "Picture of Simple Computer" [ref=e232]
            - generic [ref=e233]:
              - heading "Simple Computer" [level=2] [ref=e234]:
                - link "Simple Computer" [ref=e235]:
                  - /url: /simple-computer
              - generic "395 review(s)" [ref=e236]
              - generic [ref=e239]:
                - generic [ref=e241]: "800.00"
                - button "Add to cart" [ref=e243] [cursor=pointer]
  - generic [ref=e244]:
    - generic [ref=e245]:
      - generic [ref=e246]:
        - heading "Information" [level=3] [ref=e247]
        - list [ref=e248]:
          - listitem [ref=e249]:
            - link "Sitemap" [ref=e250]:
              - /url: /sitemap
          - listitem [ref=e251]:
            - link "Shipping & Returns" [ref=e252]:
              - /url: /shipping-returns
          - listitem [ref=e253]:
            - link "Privacy Notice" [ref=e254]:
              - /url: /privacy-policy
          - listitem [ref=e255]:
            - link "Conditions of Use" [ref=e256]:
              - /url: /conditions-of-use
          - listitem [ref=e257]:
            - link "About us" [ref=e258]:
              - /url: /about-us
          - listitem [ref=e259]:
            - link "Contact us" [ref=e260]:
              - /url: /contactus
      - generic [ref=e261]:
        - heading "Customer service" [level=3] [ref=e262]
        - list [ref=e263]:
          - listitem [ref=e264]:
            - link "Search" [ref=e265]:
              - /url: /search
          - listitem [ref=e266]:
            - link "News" [ref=e267]:
              - /url: /news
          - listitem [ref=e268]:
            - link "Blog" [ref=e269]:
              - /url: /blog
          - listitem [ref=e270]:
            - link "Recently viewed products" [ref=e271]:
              - /url: /recentlyviewedproducts
          - listitem [ref=e272]:
            - link "Compare products list" [ref=e273]:
              - /url: /compareproducts
          - listitem [ref=e274]:
            - link "New products" [ref=e275]:
              - /url: /newproducts
      - generic [ref=e276]:
        - heading "My account" [level=3] [ref=e277]
        - list [ref=e278]:
          - listitem [ref=e279]:
            - link "My account" [ref=e280]:
              - /url: /customer/info
          - listitem [ref=e281]:
            - link "Orders" [ref=e282]:
              - /url: /customer/orders
          - listitem [ref=e283]:
            - link "Addresses" [ref=e284]:
              - /url: /customer/addresses
          - listitem [ref=e285]:
            - link "Shopping cart" [ref=e286]:
              - /url: /cart
          - listitem [ref=e287]:
            - link "Wishlist" [ref=e288]:
              - /url: /wishlist
      - generic [ref=e289]:
        - heading "Follow us" [level=3] [ref=e290]
        - list [ref=e291]:
          - listitem [ref=e292]:
            - link "Facebook" [ref=e293]:
              - /url: http://www.facebook.com/nopCommerce
          - listitem [ref=e294]:
            - link "Twitter" [ref=e295]:
              - /url: https://twitter.com/nopCommerce
          - listitem [ref=e296]:
            - link "RSS" [ref=e297]:
              - /url: /news/rss/1
          - listitem [ref=e298]:
            - link "YouTube" [ref=e299]:
              - /url: http://www.youtube.com/user/nopCommerce
          - listitem [ref=e300]:
            - link "Google+" [ref=e301]:
              - /url: https://plus.google.com/+nopcommerce
    - generic [ref=e302]:
      - text: Powered by
      - link "nopCommerce" [ref=e303]:
        - /url: http://www.nopcommerce.com/
    - generic [ref=e304]: Copyright © 2026 Tricentis Demo Web Shop. All rights reserved.
```

# Test source

```ts
  1  | class LoginPage {
  2  | 
  3  |     constructor(page) {
  4  | 
  5  |         this.page = page;
  6  | 
  7  |         this.loginLink = page.locator('.ico-login');
  8  | 
  9  |         this.emailInput = page.locator('#Email');
  10 | 
  11 |         this.passwordInput = page.locator('#Password');
  12 | 
  13 |         this.rememberMe = page.locator('#RememberMe');
  14 | 
  15 |         this.loginButton = page.locator('input[value="Log in"]');
  16 | 
  17 |         this.logoutLink = page.locator('.ico-logout');
  18 | 
  19 |         this.errorMessage = page.locator('.validation-summary-errors');
  20 | 
  21 |         this.accountName = page.locator('.account');
  22 |     }
  23 | 
  24 | 
  25 |      async gotoLoginPage() {
  26 | 
> 27 |         await this.page.goto('https://demowebshop.tricentis.com/');
     |                         ^ Error: page.goto: Test timeout of 30000ms exceeded.
  28 | 
  29 |         await this.loginLink.click();
  30 |     }
  31 | 
  32 |     async login(email, password) {
  33 |     await this.emailInput.fill(email);
  34 |     await this.passwordInput.fill(password);
  35 | 
  36 |       await this.loginButton.click();
  37 |     
  38 | }
  39 | 
  40 | 
  41 |     async loginWithRememberMe(email, password) {
  42 | 
  43 |         await this.emailInput.fill(email);
  44 | 
  45 |         await this.passwordInput.fill(password);
  46 | 
  47 |         await this.rememberMe.check();
  48 | 
  49 |         await this.loginButton.click();
  50 |     }
  51 | 
  52 | 
  53 |     async logout() {
  54 | 
  55 |         await this.logoutLink.click();
  56 |     }
  57 | 
  58 | 
  59 | 
  60 |     async getErrorText() {
  61 | 
  62 |         return await this.errorMessage.textContent();
  63 |     }
  64 | }
  65 | 
  66 | module.exports = LoginPage;
```